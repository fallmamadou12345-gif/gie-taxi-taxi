...
function makeWrapper(sqlDb) {
  _db = sqlDb;
  return {
    exec(sql) { sqlDb.run(sql); saveDB(); return this; },
    prepare(sql) {
      return {
        run(...p) {
          const s = sqlDb.prepare(sql); s.run(p); s.free(); saveDB();
        ...
        get(...p) {
          ...
          if (s.step()) {
            const o={}; s.getColumnNames().forEach((c,i)=>o[c]=s.get()[i]);
            s.free(); return o;
          }
          s.free(); return undefined;
        },
        all(...p) {
          const rows=[]; const s=sqlDb.prepare(sql); s.bind(p);
          while(s.step()){const o={};s.getColumnNames().forEach((c,i)=>o[c]=s.get()[i]);rows.push(o);}
          s.free(); return rows;
        }
      };
    },
...
CREATE TABLE IF NOT EXISTS produits(id INTEGER PRIMARY KEY AUTOINCREMENT,nom TEXT NOT NULL,categorie TEXT NOT NULL,icon TEXT DEFAULT '📦',prix_achat INTEGER DEFAULT 0,prix_vente INTEGER DEFAULT 0,stock INTEGER DEFAULT 0,description TEXT DEFAULT '',actif INTEGER DEFAULT 1,created_at TEXT DEFAULT(datetime('now')));
...
function seed(dbw) {
  ...
  if (!dbw.prepare('SELECT 1 FROM membres LIMIT 1').get()) {
    ...
    [[1,'DAOUDA','NDIAYE','776351010','','','2024-09-01','actif',''],[2,'GORA','GUEYE','775919566','','','2024-09-01','actif',''],[3,'CHEIKH A.B','THIAM','774414821','','','2024-09-01','actif',''],[5,'SERIGNE','CISSE','770330761','','','2024-09-01','actif',''],[7,'MAMADOU LAMINE','GOUDIABY','779186799','','','2024-09-01','suspendu','Retard'],[9,'CHERIF','CORREA','772146324','','','2024-09-01','actif',''],[10,'DAOUDA','DIOUF','777033600','768889920','','2024-09-01','actif',''],[11,'AZIZ','DIONE','775409070','','','2024-09-01','actif',''],[14,'ADAMA','DIOUF','776872780','','','2024-09-01','actif',''],[16,'IBRAHIMA','SENE','775989102','','','2024-09-01','actif',''],[17,'SIDY','NIANE','770247233','','','2024-09-01','actif',''],[18,'BABA','NGOM','784706758','','','2024-09-01','actif',''],[19,'AMADOU','DIA','778116383','','','2024-09-01','suspendu',''],[22,'NAR','NDIAYE','772873155','','','2024-09-01','actif',''],[23,'YAYA','DIALLO','772075967','','','2024-09-01','actif',''],[25,'LAMP','DIOUF','777977407','','','2024-09-01','actif',''],[26,'DJIBY','GUEYE','775529648','','','2024-09-01','actif',''],[29,'MOR','DIENG','775858522','','','2024-09-01','actif',''],[30,'DAME','SARR','775738861','','','2024-09-01','actif',''],[31,'ALIOU','YADE','775360684','775703176','','2024-09-01','actif',''],[34,'BOUBACAR','DIALLO','784202067','','','2024-09-01','actif',''],[35,'MOHAMADOU','GUEYE','773815985','','','2024-09-01','actif',''],[38,'ABLAYE','NDIAYE','775345429','','','2024-09-01','actif',''],[49,'DAME','FALL','782083918','771938305','','2024-09-01','actif',''],[50,'CHEIKH','FALL','779399081','','','2024-09-01','actif',''],[55,'MALICK','MBAYE','774283390','','','2024-09-01','actif',''],[60,'BABACAR','NGOM','776592907','','','2024-09-01','actif',''],[66,'MAKHTAR','FALL','774526677','','','2024-09-01','actif',''],[71,'AMADOU','SECK','772418248','','','2024-09-01','actif',''],[90,'MAMADOU','FALL','772449508','','','2024-09-01','actif','Modèle'],[95,'TOUBA','WADE','761258546','','','2024-09-01','actif',''],[103,'IBRAHIMA','SY','777264140','','','2024-09-01','actif',''],[125,'MBAYE','GUEYE','764608964','','','2024-09-01','actif','']].forEach(m=>ins.run(...m,dp));
    console.log('✅ 33 membres créés');
  }
  if (!dbw.prepare('SELECT 1 FROM cotisations LIMIT 1').get()) {
    ...
    console.log('✅ Cotisations importées');
  }
