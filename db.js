...
async function forceSeed() {
  ...
  try {
    // Staff
    if (!dbw.prepare('SELECT COUNT(*) as n FROM staff').get().n) {
      dbw.prepare('INSERT OR IGNORE INTO staff(username,pin_hash,role,nom)VALUES(?,?,?,?)').run('directeur',h0,'directeur','Directeur GIE');
      dbw.prepare('INSERT OR IGNORE INTO staff(username,pin_hash,role,nom)VALUES(?,?,?,?)').run('caissier',hc,'caissier','Caissier GIE');
      dbw.prepare('INSERT OR IGNORE INTO staff(username,pin_hash,role,nom)VALUES(?,?,?,?)').run('president',h0,'president','Président GIE');
    }
    // Produits
    if (!dbw.prepare('SELECT COUNT(*) as n FROM produits').get().n) {
      const ins=dbw.prepare('INSERT INTO produits(nom,categorie,icon,prix_achat,prix_vente,stock,description)VALUES(?,?,?,?,?,?,?)');
      [['Batterie SOLANCE','BATTERIE','🔋',33000,45000,7,'12V'],['Batterie SISA','BATTERIE','🔋',27000,40000,5,''],['Smartphone Spark 40','TELEPHONE','📱',57000,80000,3,'128Go'],['Smartphone A16','TELEPHONE','📱',74000,100000,2,'Samsung'],['Smartphone POP 10','TELEPHONE','📱',47500,65000,1,''],['Huile moteur 5L','HUILE','🛢️',9500,13000,0,'5L'],['Assurance taxi 1 mois','ASSURANCE','🛡️',0,21000,0,'']].forEach(p=>ins.run(...p));
    }
    // Membres
    if (!dbw.prepare('SELECT COUNT(*) as n FROM membres').get().n) {
      const ins=dbw.prepare('INSERT OR IGNORE INTO membres(id,prenom,nom,telephone,tel2,taxi,adhesion,statut,notes,pin_hash)VALUES(?,?,?,?,?,?,?,?,?,?)');
      ...
      .forEach(([id,p,n,t,t2])=>ins.run(id,p,n,t,t2||'','','2024-09-01','actif','',dp));
    }
    saveDB();
    const n=dbw.prepare('SELECT COUNT(*) as n FROM membres').get().n;
    const s=dbw.prepare('SELECT COUNT(*) as n FROM staff').get().n;
    console.log(`✅ forceSeed OK: ${n} membres, ${s} staff`);
    return {membres:n, staff:s};
