...
const { getDB, saveDB, reloadFromDisk } = require('./db');
...
// Sauvegarde automatique après chaque écriture
app.use((req, res, next) => {
  const orig = res.json.bind(res);
  res.json = (body) => {
    const result = orig(body);
    if (['POST','PUT','PATCH','DELETE'].includes(req.method) && !req.path.includes('/login')) {
      setImmediate(saveDB); // sauvegarde non-bloquante après réponse
    }
    return result;
  };
  next();
